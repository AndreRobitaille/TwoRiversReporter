...
- **Admin styling lives in `app/assets/stylesheets/admin.css`**, loaded only by `layouts/admin.html.erb` and always *after* `application.css`, from which it inherits every token. It has four numbered sections — density base, shell/sidebar, components, and a transitional utility layer that Phase 2 deletes. Never put admin rules in `application.css`; never put a rule below the section-4 header (a set-equality test guards that boundary). Admin runs at 13px "Instrument" density: hairline rules, `--font-data` for IDs/timestamps/durations so columns align, and colour reserved for state.
- **`Admin::Navigation` is the single source of truth for admin navigation.** The sidebar partial and the dashboard launcher both render `Admin::Navigation::GROUPS`; tests assert they cannot list different destinations. Adding an admin page means adding an entry there, not editing two lists. Surfaces reached in context are deliberately absent from it: generated images (from a topic or meeting), membership applications (from a user), personal security settings (the sidebar foot).
- **Scoped CSS rules out-specify unscoped ones, and this has bitten repeatedly.** `.theme-silo .btn` (0,2,0) beats `.btn--secondary` (0,1,0); `.theme-silo h1` (0,1,1) beats `.mb-1` (0,1,0); `.theme-silo table thead th` (0,1,3) beats `.text-right`. Roughly a dozen live collisions were found this way. Before adding a scoped rule, work out what else matches the same element and who wins — and be aware `test/assets/admin_stylesheet_test.rb` guards colour, margin/padding and `text-align` across `p h1-h4 td th`, but *not* `font-size`/`font-weight` or other element selectors.
- **Never call `Admin::TopicsHelper#topic_recent_mentions` in a list context.** It eager-loads each linked document's `extracted_text` and every `Extraction` behind it. Calling it once per row on the topics index cost 23.6 seconds and 733 queries against the 641-topic development database. Mention previews load on expansion via a lazy `turbo_frame_tag` against `mention_preview_admin_topic_path`; a query-count test guards it.
