...
- **`GeneratedImages::*`** — Services/jobs that generate, retry, and repair civic images; all OpenAI usage still routes through `Ai::OpenAiService`.
...
- Top-level: `ExtractTopicsJob` (runs on agenda parse AND minutes arrival; prefers minutes text over packet), `ExtractVotesJob`, `ExtractCommitteeMembersJob`, `SummarizeMeetingJob` (accepts `mode:` kwarg — `:full` default runs meeting summary + topic summaries + prune + knowledge extraction; `:agenda_preview` runs only the meeting-level summary from `agenda_pdf` text plus a topic-briefing refresh pass, triggered by `AnalyzePdfJob` with 5-min delay after agenda_pdf analysis), `PruneHollowAppearancesJob` (detaches hollow standing-slot topic appearances using the `activity_level` field in `item_details`; enqueued automatically at the end of `SummarizeMeetingJob` in `:full` mode), `IngestKnowledgeSourceJob`
